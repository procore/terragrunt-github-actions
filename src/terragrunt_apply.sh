...
function terragruntApply {
  ...
  applyOutput=$(${tfBinary} apply-all -auto-approve -input=false --terragrunt-non-interactive${*} 2>&1)
  applyExitCode=${?}
  ...
  if [ "$GITHUB_EVENT_NAME" == "pull_request" ] && [ "${tfComment}" == "1" ]; then
    applyCommentWrapper="#### \`${tfBinary} apply-all --terragrunt-non-interactive\` ${applyCommentStatus}
